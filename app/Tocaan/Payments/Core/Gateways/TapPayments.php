...
class TapPayments extends PaymentsActions implements GatewayContract
{
    const DRIVER = 'tap';
    ...
    public function pay(null|array $credentials): null|RedirectResponse
    {
        ...
        if( $result->ok() )
        {
            if( !is_null($result->body()) )
            {
                ...
                if( !is_null($body->status) && $body->status=='INITIATED' && !is_null($body->transaction->url) )
                {
                    return redirect()->to($body->transaction->url);
                }
            }
        }
    ...
    protected function fields()
    {
        $fields =
        [
            'amount' => number_format($this->total ?? $this->invoice->total, 4),
            'currency' => $this->currency ?? 'KWD',
            'description' => $this->product_title ?? $this->invoice->description,
            'customer' =>
            [
                'email' => $this->client_email ?? $this->invoice->user->email,
                'first_name' => strtok($this->client_name ?? $this->invoice->user->name, ''),
                'last_name' => '',
                'phone' =>
                [
                    'number' => $this->client_mobile ?? $this->invoice->user->mobile
                ]
            ],
            'source' =>
            [
            ...
            'redirect' =>
            [
                'url' => $this->success_url ?? route('payments.callback',
                [
